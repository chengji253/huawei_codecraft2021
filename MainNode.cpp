    ...
    */
    ...
    if(virtualHost.nodeNum)  // 双节点分布
    {
        for(ui i = 0; i < inputs->serverTypes.size(); i++)
        {
            ...
            if(virtualHost.cpuNum / 2 <= inputs->serverTypes[i].cpuNum / 2
                && virtualHost.memorySize / 2 <= inputs->serverTypes[i].memorySize / 2)
            {
    ...
    }else
    {
        for(ui i = 0; i < inputs->serverTypes.size(); i++)
        {
            ...
            if(virtualHost.cpuNum <= inputs->serverTypes[i].cpuNum / 2
                && virtualHost.memorySize <= inputs->serverTypes[i].memorySize / 2)
            {
                fitableServer.emplace_back(inputs->serverTypes[i]);
    ...
    {
        double score = k1 * score_1[i] / score_1_max + k2 * score_2[i] / score_2_max 
                        + k3 * score_3[i] / score_3_max + k4 * score_4[i] / score_4_max;
...
void MainNode::Be_putIn(Request& request, bool& result, int& id)
{
    ...
    {   //如果能够装下
        ...
        if (Vm.nodeNum == 0) //单节点
        {
            if (server.fit(server.A,Vm.cpuNum, Vm.memorySize)
            && server.fit(server.B,Vm.cpuNum, Vm.memorySize))
            {
            ...
            else if (!server.fit(server.A,Vm.cpuNum, Vm.memorySize)
                     && server.fit(server.B,Vm.cpuNum, Vm.memorySize))
            {
        ...
        {
            if (server.fit(server.A,Vm.cpuNum/2, Vm.memorySize/2)
                && server.fit(server.B,Vm.cpuNum/2, Vm.memorySize/2))
            {
                left = (server.A.first+server.B.first -Vm.cpuNum
                        + server.A.second+server.B.second-Vm.memorySize);
                left_compare.insert({left, server.id});
            }
        }
    }
...
void MainNode::distribute_now() {
    ...
    // 生成要购买的服务器序列
    for(Request& request:requests){//遍历当天所有的请求
    ...
    for(Request& request:requests){
        if(request.operation == ADD){
            ...
            }else inputs->addOutput(tmpStr+", B)");
        }
    }
...
void MainNode::migrate() {
    //inputs->addOutput("(migration, 0)");
     ...
     int migrate_constraint = int(vmOnserver.size())*3/1000; //-1
            ...
            for(int j=Servers_need_adjustment.size()-1;j>=0;j--)
            {
                ...
                if (Vm_type.nodeNum==0) //单节点
                {
                        ...
                        server_now->fit(server_now->B,Vm_type.cpuNum, Vm_type.memorySize))
                    {
...
#endif
                        int resultA
                        = server_now->A.first-Vm_type.cpuNum + server_now->A.second-Vm_type.memorySize;
                        int resultB
                        = server_now->B.first-Vm_type.cpuNum + server_now->B.second-Vm_type.memorySize;
                        //放在剩的少的那里
                        if (resultA >= resultB)
                        {
                        ...
                        else
                        {
                            ...
                            server_now->requestIDs.insert(Vm_name);
                            std::string s
                            =Output_Migration_single(Vm_name, *server_now, DEPLOY_A);
                ...
                else //双节点
                {
                        ...
                        server_now->fit(server_now->B,Vm_type.cpuNum/2, Vm_type.memorySize/2))
                    {
                        ...
                        std::string s
                        =Output_Migration_double(Vm_name, *server_now);
...
void MainNode::AdjustmentTwo(std::vector<double> &Servers_score, int migrate_constraint)
{
    ...
    */
    ...
    {//从分低的开始计算
        ...
        if (server_now->A.first <= 10 || server_now->A.second<= 10
        || server_now->B.first <= 10 || server_now->B.first <= 10)
            continue;
    ...
    while (Vm_all.size() != 0)
    {
        ...
        while (Vm_sign != Vm_left)
        {
            ...
            for (auto Vm:Vm_all) //每次都会计算当前分配的分数 然后选择最高分的那个
            {
                auto Vm_type = inputs->virtualHosts[inputs->requestIDMap[Vm]];
                auto r
                = Compute_Vm_score_all(*server, Vm_type, Vm);
            ...
            else if (VmS.second.first == DEPLOY_A || VmS.second.first == DEPLOY_B)
            {
                ...
                if (Vm_all.size() != 0)
                { Vm_all.erase(Vm_all.begin() + itq); }//删除这个元素
            ...
            else    //双节点
            {
                ...
                std::string s
                =Output_Migration_double(VmS.second.second, currentServers[Servers_need_adjustment[ser_i]]);
                if(s.size()!=0)
                {   //如果位置被改变了
#ifdef Migration_print
                    cout<<s<<endl;
